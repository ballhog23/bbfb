...
export async function handlerServeIndex(req: Request, res: Response) {
    ...
    const seasonJustFinished =
        !leagueState.isLeagueActive &&
        leagueState.season === config.league.season;
    const completedLeagueId = seasonJustFinished
        ? config.league.id
        : config.league.prevId;
    const completedLeagueSeason = seasonJustFinished
        ? config.league.season
        : config.league.prevSeason;
