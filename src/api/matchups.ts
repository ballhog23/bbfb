...
import {
    ...
    selectLeagueMatchupsByWeekWithoutByes,
...
export async function handlerGetLeagueMatchups(
    req: Request<MatchupParams>,
    res: Response
) {
    ...
    if (matchups.length === 0)
        throw new BadRequestError(
            `No matchups found for League ID: ${leagueId}`
        );
...
export async function handlerGetLeagueMatchupsByWeek(
    req: Request<MatchupParams>,
    res: Response
) {
    const params = req.params;
    ...
    if (isNaN(week) || !Number.isInteger(week) || week <= 0 || week > 17)
        throw new BadRequestError(
            "You must provide a valid week, ranging 1-17"
        );

    const ungroupedMatchups = await selectLeagueMatchupsByWeekWithoutByes(
        leagueId,
        week
    );
    if (ungroupedMatchups.length === 0)
        throw new BadRequestError(
            `No matchups found for League ID: ${leagueId} during Week: ${week}`
        );
...
export async function handlerGetSpecificLeagueMatchup(
    req: Request<MatchupParams>,
    res: Response
) {
    const params = req.params;
    ...
    if (
        ...
        throw new BadRequestError(
            "You must provide a valid matchup number. Ranging 1-6"
        );

    const matchups = await selectSpecificLeagueMatchup(
        leagueId,
        week,
        matchupId
    );
