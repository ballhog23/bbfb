...
// working
export async function handlerGetLeagueUsers(
    req: Request<LeagueUserParams>,
    res: Response
) {
    ...
    if (allUsers.length === 0)
        throw new NotFoundError(
            `No league users found for League ID: ${leagueId}.`
        );
...
// working
export async function handlerGetLeagueUser(
    req: Request<LeagueUserParams>,
    res: Response
) {
    const params = req.params;
    ...
    if (!user)
        throw new NotFoundError(
            `User with ID: ${userId} not found in League ID: ${leagueId}.`
        );
