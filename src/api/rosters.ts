...
export async function handlerGetAllRosters(_: Request, res: Response) {
    ...
    if (rosters.length === 0)
        throw new NotFoundError("No league rosters found.");
...
export async function handlerGetLeagueUserRosters(
    req: Request<RosterParams>,
    res: Response
) {
    ...
    if (rosters.length === 0)
        throw new NotFoundError(
            `Could not find rosters for User ID: ${userId}`
        );
...
// per season
export async function handlerGetLeagueRosters(
    req: Request<RosterParams>,
    res: Response
) {
    const params = req.params;
    ...
    if (rosters.length === 0)
        throw new NotFoundError(
            `Rosters for League ID: ${leagueId} not found.`
        );
...
export async function handlerGetLeagueUserRoster(
    req: Request<RosterParams>,
    res: Response
) {
    const params = req.params;
    ...
    if (rosters.length === 0)
        throw new NotFoundError(
            `Roster for User ID: ${userId} not found in League ID: ${leagueId}`
        );
