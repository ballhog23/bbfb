...
import {
    selectAllSleeperUsersData,
    getHeadToHeadMatchupData,
    type HeadToHeadData,
...
export async function buildDropDownTeams() {
    ...
    if (teams.length === 0)
        throw new NotFoundError(
            "No teams found to populate dropdown selections"
        );
...
export function assembleMatchupData(matchupData: HeadToHeadData) {
    ...
    const partialStats = matchupData.reduce(
        (acc, game) => {
            ...
            // low scores
            if (
                parsedChallengerPoints < acc.challengerLowScore ||
                acc.challengerLowScore === 0
            )
                acc.challengerLowScore = parsedChallengerPoints;

            if (
                parsedOpponentPoints < acc.opponentLowScore ||
                acc.opponentLowScore === 0
            )
                acc.opponentLowScore = parsedOpponentPoints;

            // closest game
            const gamePointDiff = Math.abs(
                parsedChallengerPoints - parsedOpponentPoints
            );
    ...
    const stats = {
        ...partialStats,
        challengerAvgPoints: (
            partialStats.challengerTotal / totalMatchupCount
        ).toFixed(2),
        opponentAvgPoints: (
            partialStats.opponentTotal / totalMatchupCount
        ).toFixed(2),
        challengerWinAvg: Math.round(
            (partialStats.challengerWins / totalMatchupCount) * 100
        ),
        opponentWinAvg: Math.round(
            (partialStats.opponentWins / totalMatchupCount) * 100
        ),
