import {
    SelectMatchupOutcome,
    StrictInsertMatchupOutcome,
} from "../../db/schema.js";
import {
    selectLeagueMatchupOutcomes,
    insertMatchupOutcome,
...
export async function insertLeagueMatchupOutcomes(
    matchups: StrictInsertMatchupOutcome[]
) {
...
async function buildLeagueMatchupOutcomes() {
    ...
    const result = matchupsPerLeague
        .map((matchups) => normalizeMatchupOutcome(matchups))
        .flat();
...
function normalizeMatchupOutcome(matchups: StrictMatchupOutcome[]) {
    ...
    for (const matchup of matchups) {
        ...
        if (matchupId === null)
            normalizedMatchups.push({ ...matchup, outcome: "BYE" });
        else if (pointsFor === pointsAgainst)
            normalizedMatchups.push({ ...matchup, outcome: "T" });
        else if (pointsFor > pointsAgainst)
            normalizedMatchups.push({ ...matchup, outcome: "W" });
