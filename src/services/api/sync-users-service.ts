...
import {
    buildCurrentLeagueUsers,
    syncLeagueUsers,
} from "./league-users-service.js";

export async function syncUsers() {
    const dbLeagueUsersIds = (await selectAllLeagueUsers()).map(
        (user) => user.userId
    );
    ...
    const syncedSleeperUsers = await syncSleeperUsers([
        ...dbLeagueUsersIds,
        ...newSleeperUsersIds,
    ]);
