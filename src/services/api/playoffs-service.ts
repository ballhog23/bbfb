...
import {
    ...
    type NullableRawBracketMatchup,
    RawBracketMatchup,
    NullableTeamFromMatchup,
} from "../../lib/zod.js";
import {
    selectPlayoffMatchups,
    type TempPlayoffMatchupRow,
...
export async function syncPlayoffMatchups() {
    ...
    const rawBrackets = await buildBracketMapsForLeague(
        leagueId,
        dbPlayoffMatchups
    );
...
async function insertPlayoffBracketMatchups(
    bracketMatchups: StrictInsertPlayoffMatchup[]
) {
    ...
    for (let i = 0; i < bracketMatchups.length; i += CHUNK_SIZE) {
        ...
        const currentInsert = chunk.map((matchup) =>
            insertPlayoffMatchup(matchup)
        );
...
async function buildPlayoffBracketHistory() {
    ...
    const normalizedHistory =
        rawToNormalizedPlayoffBracketMatchups(playoffHistory);
...
export async function getAllPlayoffBracketsHistory(): Promise<RawBracketMap[]> {
    ...
    const leagueIds = Array.from(
        new Set(playoffMatchups.map((m) => m.leagueId))
    );

    const historicalBracketsRaw = await Promise.all(
        leagueIds.map(async (leagueId) =>
            buildBracketMapsForLeague(leagueId, playoffMatchups)
        )
    );
...
async function buildBracketMapsForLeague(
    ...
    return Promise.all(
        bracketTypes.map(async (bracketType) => {
            const bracketMatchups = await sleeper.getLeaguePlayoffBracket(
                bracketType,
                leagueId
            );

            // Step 1: Map each Sleeper bracket matchup to our shape, resolving matchupId from DB
            const mappedMatchups: RawBracketWithMatchupId[] =
                bracketMatchups.map((matchup) => {
                    ...
                    if (matchingMatchupIds.length === 1) {
                        ...
                        console.warn(
                            "Multiple matchupIds found for matchup:",
                            matchup,
                            matchingMatchupIds
                        );
                        matchupId = matchingMatchupIds[0];
...
function normalizePlayoffBracketMatchup(
    ...
    if (matchup.t1_from && isWinner(matchup.t1_from))
        t1FromWinner = matchup.t1_from.w;
    if (matchup.t1_from && isLoser(matchup.t1_from))
        t1FromLoser = matchup.t1_from.l;
    if (matchup.t2_from && isWinner(matchup.t2_from))
        t2FromWinner = matchup.t2_from.w;
    if (matchup.t2_from && isLoser(matchup.t2_from))
        t2FromLoser = matchup.t2_from.l;
...
function rawToNormalizedPlayoffBracketMatchups(
    bracketMaps: RawBracketMap[]
): StrictInsertPlayoffMatchup[] {
    const matchups = bracketMaps.flatMap((bracket) => {
        ...
        const normalized = bracket.bracketMatchups.map(
            (matchup) =>
                undefinedToNullDeep(matchup) as NullableRawBracketMatchup
        );

        return normalized.map((matchup) =>
            normalizePlayoffBracketMatchup(matchup, bracketType, leagueId)
        );
    });
