import type {
    SelectNFLPlayer,
    StrictInsertNFLPlayer,
} from "../../db/schema.js";
import { Sleeper } from "../../lib/sleeper.js";
import { insertNFLPlayer } from "../../db/queries/players.js";
import {
    strictNFLPlayerSchema,
    type RawNFLPlayer,
    NullableRawNFLPlayer,
    StrictNFLPLayer,
} from "../../lib/zod.js";
...
export async function insertNFLPlayers(players: StrictInsertNFLPlayer[]) {
    ...
    if (failedPlayers.length > 0) {
        failedPlayers.forEach((player) =>
            console.error(
                `Player ID: ${player.playerId} failed with ${player.error}`
            )
        );
    }
...
export function normalizeNFLPlayer(rawPlayer: RawNFLPlayer) {
    const fantasyPositions = rawPlayer.fantasy_positions
        ? rawPlayer.fantasy_positions.map((position) =>
              normalizeString(position)
          )
        : null;
    const position = rawPlayer.position
        ? normalizeString(rawPlayer.position)
        : null;
    const team = rawPlayer.team ? normalizeString(rawPlayer.team) : null;
    const number = rawPlayer.number ? rawPlayer.number : null;
    const injuryStatus = rawPlayer.injury_status
