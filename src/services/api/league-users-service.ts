import type {
    SelectLeagueUser,
    StrictInsertLeagueUser,
} from "../../db/schema.js";
import { Sleeper } from "../../lib/sleeper.js";
import {
    strictLeagueUserSchema,
    type RawLeagueUser,
    NullableRawLeagueUser,
    StrictLeagueUser,
} from "../../lib/zod.js";
...
export async function syncLeagueUsers(
    currentLeagueUsers: StrictInsertLeagueUser[]
) {
    return await insertLeagueUsers(currentLeagueUsers);
}

export async function buildAndInsertLeagueUserHistory(
    leagueUsers: StrictInsertLeagueUser[]
) {
    return await insertLeagueUsers(leagueUsers);
}

export async function buildCurrentLeagueUsers(): Promise<
    StrictInsertLeagueUser[]
> {
    ...
    return rawToNormalizedLeagueUsers([
        {
            ...
            leagueUsers,
        } satisfies RawCurrentLeagueUsersMap,
    ]);
}
...
export async function buildLeagueUsersHistory() {
    const leagueHistoryIds = (await selectAllLeagues()).map(
...
export async function getAllLeagueUsersHistory(
    leaguesIds: string[]
): Promise<RawLeagueUsersMap[]> {
    const sleeper = new Sleeper();
    const usersByLeague = await Promise.all(
        leaguesIds.map(async (leagueId) => ({
            leagueId,
            leagueUsers: await sleeper.getLeagueUsers(leagueId),
        }))
    );
...
export function normalizeLeagueUser(
    rawUser: NullableRawLeagueUser,
    leagueId: string
) {
    const teamName = rawUser.metadata?.team_name
        ? normalizeString(rawUser.metadata.team_name)
        : null;
    const avatarId = rawUser.metadata?.avatar
        ? normalizeString(rawUser.metadata.avatar)
        : null;
