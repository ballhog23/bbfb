...
function trendingUpSvg(): string {
    return svgIcon('trending-up');
}
...
function renderCard(card: StatsCard): string {
    const badge = card.badge
        ? `<span class="stats-card__badge">${trendingUpSvg()}${escapeHTML(card.badge)}</span>`
        : '';

    const roast = card.roast
        ? `<p class="stats-card__roast">${escapeHTML(card.roast)}</p>`
        : '';

    return `
        <div class="stats-card stats-card--${escapeHTML(card.variant || 'default')}">
            <div class="stats-card__header">
                <span class="stats-card__title">${escapeHTML(card.title)}</span>
                <div class="stats-card__icon">${svgIcon(card.icon)}</div>
            </div>
            <div class="stats-card__body">
                <div class="stats-card__value-row">
                    <span class="stats-card__value">${escapeHTML(String(card.value))}</span>
                    ${badge}
                </div>
                <p class="stats-card__desc">${escapeHTML(card.desc)}</p>
                ${roast}
            </div>
        </div>`;
}

function renderSection(section: StatsSection): string {
    return `
        <section class="stats-section">
            <div class="section-header">
                <div class="section-header__icon">${svgIcon(section.icon)}</div>
                <div class="section-header__text">
                    <h2>${escapeHTML(section.title)}</h2>
                    <p>${escapeHTML(section.subtitle)}</p>
                </div>
            </div>
            <div class="stats-grid stats-grid--${section.cols}">
