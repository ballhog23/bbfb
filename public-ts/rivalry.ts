...
const challengerSelect =
    document.querySelector<HTMLSelectElement>("#challenger")!;
...
const resultsContainer =
    document.querySelector<HTMLElement>("#rivalry-results")!;
...
document
    .querySelectorAll<HTMLElement>(".team-select .team-avatar")
    .forEach((avatar) => {
...
function handleSelectChange(event: Event) {
    ...
    other.querySelector<HTMLOptionElement>(
    ...
    const avatars = container.querySelectorAll<HTMLImageElement>(
        ".team-avatar-option"
    );
    ...
    const selected = container.querySelector<HTMLImageElement>(
        `.team-avatar-option[data-user-id="${changed.value}"]`
    );
    ...
    battleBtn.title = bothSelected
        ? ""
        : "You must select both teams to begin a battle";
}
...
function randomizeAnimationDelays(container: HTMLElement) {
    ...
    container
        .querySelectorAll<HTMLElement>(".highlight-card svg")
        .forEach((svg) => {
...
function renderScoreboard(
    ...
    leftSide.appendChild(
        node(
            "div",
            `scoreboard-wins${leftLeading ? " leading" : ""}`,
            String(leftWins)
        )
    );
    ...
    rightSide.appendChild(
        node(
            "div",
            `scoreboard-wins${rightLeading ? " leading" : ""}`,
            String(rightWins)
        )
    );
    ...
    const barLeft = node(
        "div",
        `scoreboard-bar-left ${leftLeading || tied ? "bar-winner" : "bar-loser"}`
    );
    barLeft.style.width = `${leftPct}%`;
    const barRight = node(
        "div",
        `scoreboard-bar-right ${rightLeading ? "bar-winner" : "bar-loser"}`
    );
...
function renderBreakdown(data: RivalryData): HTMLElement {
    ...
    const stats = [
        {
            left: data.challengerTotal,
            label: "Total Points",
            right: data.opponentTotal,
        },
        {
            left: data.challengerAvgPoints,
            label: "Avg Points",
            right: data.opponentAvgPoints,
        },
        {
            left: data.challengerHiscore,
            label: "High Score",
            right: data.opponentHiscore,
        },
        {
            left: data.challengerLowScore,
            label: "Low Score",
            right: data.opponentLowScore,
        },
        {
            left: `${data.challengerWinAvg}%`,
            label: "Win %",
            right: `${data.opponentWinAvg}%`,
        },
    ];
...
function renderHighlights(data: RivalryData): HTMLElement {
    ...
    const combined = (
        parseFloat(data.challengerTotal) + parseFloat(data.opponentTotal)
    ).toFixed(2);
    const avgPerGame = (
        (parseFloat(data.challengerTotal) + parseFloat(data.opponentTotal)) /
        totalGames
...
function renderMatchRow(match: MatchupRow): HTMLElement {
    ...
    resultEl.appendChild(
        node("span", result === "W" ? "result-win" : "result-loss", result)
    );
